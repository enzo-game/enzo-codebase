...
**一次做一個小塊檔**；`<id>.jpg` 已存在就跳過。

**總需求：809 張新卡圖。目前已上線 459 張（57%）。**

| 批次 | 小塊檔 | 狀態 |
|---|---|---|
| 批次 1 | `batch-1-part-01..07` | ✅ 全部完成上線（194） |
| 批次 2 | `batch-2-part-01..06` | ✅ 全部完成上線（175） |
| 批次 3 | `batch-3-part-01..03` | ✅ 完成上線（90） |
| 批次 3 | `batch-3-part-04, 05` | ⏳ **下一步請生這裡** |
| 批次 4 | `batch-4-part-01..05` | ⏳ 未開始（140） |
| 批次 5 | `batch-5-part-01..05` | ⏳ 未開始（150） |

> 若你的紀錄還停在很舊的「ORDER-17」：那已被卡池大改版取代，作廢。以 `docs/card-art/` 為唯一權威。
> 詳細現況另見 `docs/card-art-status-report.md`。

**接下來請做**：打開 `docs/card-art/README.md`，從 **`batch-3-part-04.md`** 開始，一次生完一個小塊檔的每一張，存成 `public/images/cards/<id>.jpg`，做完換下一檔，做完幾檔就告知工程端接線。
